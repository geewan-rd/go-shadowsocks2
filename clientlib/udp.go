...
func udpSocksLocal(laddr, server string, shadow func(net.PacketConn) net.PacketConn) error {
	...
	udpPacketConn, err = net.ListenPacket("udp", laddr)
	if err != nil {
		logf("UDP local listen error: %v", err)
		return err
	}
	...
	go func() {
		...
		for {
			...
			case <-chExit:
				...
				return
			default:
				n, raddr, err := udpPacketConn.ReadFrom(buf)
				if err != nil {
					logf("UDP local read error: %v", err)
					continue
				}
				pc := nm.Get(raddr.String())
				if pc == nil {
					pc, err = net.ListenPacket("udp", localIP)
					if err != nil {
						logf("UDP local listen error: %v", err)
						continue
					}
					...
					pc = shadow(pc)
					nm.Add(raddr, udpPacketConn, pc, socksClient)
				}
